...
macOS（Apple Silicon）可直接 `cargo run -p omarchy-bible`；CI 的 `macos-latest` job 會跑 `cargo test --workspace --locked` 與 `cargo build --release -p omarchy-bible --locked`。

## 專案結構

```
crates/bible-core     資料模型、66 卷目錄、章節載入、經文搜尋、測試
crates/bible-ui       GPUI Kit：頂欄書卷/章節選擇、閱讀區、設定、搜尋、希伯來文 stub
...
- Linux：`~/.config/omarchy-bible/settings.json`（若設定了 `XDG_CONFIG_HOME` 則用該目錄）
- macOS：`~/Library/Application Support/omarchy-bible/settings.json`

欄位：`theme`（dark / light / system）、`font_size`、`view_mode`。

跟隨系統時：

- **Linux**：先看 Omarchy 主題目錄 `~/.local/state/omarchy/current/theme`（或 `XDG_STATE_HOME`）：有 `light.mode` 視為淺色；否則讀 `colors.toml` 的 `mode`。沒有 Omarchy 再用 gpui 視窗外觀（XDG portal）。再不行就回退深色。
- **macOS**：只用 gpui 視窗外觀 / `ThemeMode`，**不會**讀 Omarchy 路徑。
...
Omarchy／Arch 與 macOS Release 資產見 [packaging.md](packaging.md)。
